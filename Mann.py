...
def loss_function(preds, labels):
    ...
    #############################
    #### YOUR CODE GOES HERE ####
    return tf.nn.softmax_cross_entropy_with_logits(labels= tf.squeeze(labels[:,-1,:,:]), logits=tf.squeeze(preds[:,-1,:,:]))
    #############################
...
class MANN(tf.keras.Model):
    ...
    def call(self, input_images, input_labels):
        ...
        #############################
        #### YOUR CODE GOES HERE ####
        input_labels = tf.concat([input_labels[:,:-1,:,:],tf.zeros((tf.shape(input_labels)[0],1,self.num_classes,self.num_classes),dtype=tf.float32)],axis=1)
        concatenated_input = tf.reshape(tf.concat([input_images,input_labels],axis=3),(tf.shape(input_labels)[0],self.num_classes*(self.samples_per_class),784+self.num_classes))

        out = self.layer2(self.layer1(tf.convert_to_tensor(concatenated_input, np.float32)))
        out = tf.reshape(out,(-1,FLAGS.num_samples + 1,FLAGS.num_classes,FLAGS.num_classes))
        #############################
        return out
